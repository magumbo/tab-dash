...
var ngEventProvider = function (grid, $scope, domUtilityService, $timeout) {
    ...
    self.dragStart = function(evt){		
      ...
      evt.dataTransfer.setData('text', ''); //cannot be empty string
		grid.$groupPanel.addClass('visible');
		$(document).on('dragend', function(event) {
			if($scope.configGroups.length == 0){
				grid.$groupPanel.removeClass('visible');
			}
		});
		/*
		$scope.$watch('configGroups', function (groups) {
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    $scope.topPanelHeight = function() {
		...
		return self.config.headerRowHeight;
    };
...
window.ngGrid.i18n['en'] = {
    ...
    ngGroupPanelDescription: 'Drop here to group by that column.',
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('footerTemplate.html', //Lt changes
    ...
    "\n" +
    "        <div ng-class=\"{'disabled': largeAccount}\" style=\"float:left; margin-right: 10px;\" class=\"ngRowCountPicker\">\r" +
    "\n" +
    "            <span style=\"float: left; margin-top: 3px;\" class=\"ngLabel\">{{i18n.ngPageSizeLabel}}</span>\r" +
    "\n" +
    "            <select ng-disabled=\"largeAccount\" style=\"float: left;height: 27px; width: 100px\" ng-model=\"pagingOptions.pageSize\" >\r" +
    "\n" +
  ...
  $templateCache.put('gridTemplate.html',
    ...
    "\n" +
    "    <div class=\"ngGroupPanel\" ng-class=\"{visible: configGroups.length > 0}\" ng-show=\"showGroupPanel()\" ng-style=\"groupPanelStyle()\">\r" +
    "\n" +
